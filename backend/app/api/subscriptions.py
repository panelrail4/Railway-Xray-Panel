...
def endpoint():
    ...
    raise HTTPException(409, 'Set RAILWAY_PUBLIC_DOMAIN or PUBLIC_HOST before generating links.')
...
def make_uri(user: User, inbound: Inbound, variant: str = 'default'):
    ...
    if security == 'reality':
        ...
        params.update({
            'security': 'tls',
            'sni': os.getenv('PUBLIC_HOST') or os.getenv('RAILWAY_PUBLIC_DOMAIN') or host,
            'fp': 'chrome',
        })
    else:
    ...
    if variant == 'edge-tls' and kind == 'railway':
        params['security'] = 'tls'
        params['sni'] = os.getenv('PUBLIC_HOST') or os.getenv('RAILWAY_PUBLIC_DOMAIN') or host
        params['fp'] = 'chrome'
        port = 443
        label += '-TLS'
...
def active_links(user: User, db: Session):
    ...
    for i in inbounds:
        try:
            links.append(make_uri(user, i, 'edge-tls'))
...
def create_subscription(user_id: int, db: Session = Depends(get_db), _=Depends(require_admin)):
    ...
    public = os.getenv('RAILWAY_PUBLIC_DOMAIN') or os.getenv('PUBLIC_HOST')
    if not public:
        raise HTTPException(409, 'Set RAILWAY_PUBLIC_DOMAIN or PUBLIC_HOST.')
...
def user_links(user_id: int, db: Session = Depends(get_db), _=Depends(require_admin)):
    ...
    for i in inbounds:
        ...
        for variant in ('edge-tls', 'plain-http'):
            try: variants[variant] = make_uri(user, i, variant)
...
def one_link(user_id: int, inbound_id: int, variant: str = 'edge-tls', db: Session = Depends(get_db), _=Depends(require_admin)):
    ...
    return {'uri': make_uri(user, inbound, variant), 'variant': variant}
